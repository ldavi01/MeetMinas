...
const _elements = {
  ...
  scrollLinks: document.querySelectorAll(
    ".navbar-list__link, .footer-list__link"
  ),
...
const getImageId = (target) => {
  const arrFromChildren = Array.from(target.parentNode.children); //converte um HTMLcollection em array
...
const updateModal = (imgId) => {
  ...
  _elements.sliderThumbsImage[imgId].classList.add(
    "slider-thumbs__img--active"
  );
};
